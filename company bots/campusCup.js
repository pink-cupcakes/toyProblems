...
function campusCup(emails) {
    ...
    emails.forEach((email) => {
        let campus = email.split('@')[1];
        if (campusCount[campus]) {
            campusCount[campus] += 20;
        } else {
            campusCount[campus] = 20;
        };
    });
    ...
    let bracket4 = [];
    let bracket5 = [];
    for (var key in campusCount) {
        if (campusCount[key] < 100) {
            bracket1.push(key);
            continue;
        } else if (campusCount[key] < 200) {
            bracket2.push(key);
            continue;
        } else if (campusCount[key] < 300) {
            bracket3.push(key);
            continue;
        } else if (campusCount[key] < 500) {
            bracket4.push(key);
            continue;
        } else {
            bracket5.push(key);
    ...
    bracket4.sort();
    bracket5.sort();
    return bracket5.concat(bracket4, bracket3, bracket2, bracket1)
}